<template>
  <div class="alg-wrap">
    <div class="alg-header">
      ...
      <a href="https://gd.189.cn/plus2/h5/cmp/index.html#/" target="_blank"
        ><div class="suanli-btn flex-center">算力交易市场</div></a
      >
    </div>
    ...
    <div class="alg-table-filter">
      <div class="filters">
        <el-select
          v-model="filingTypeSelect"
          placeholder="请选择"
          @change="() => filterAlg((currentPage = 1))"
          :ref="(el) => refElements.push(el)"
        >
          <el-option
            v-for="item in filingType"
            :key="item"
            :label="item"
            :value="item"
          ></el-option>
        ...
        <el-select
          v-model="filingMonthSelect"
          placeholder="请选择"
          @change="() => filterAlg((currentPage = 1))"
          :ref="(el) => refElements.push(el)"
        >
          <el-option
            v-for="item in filingMonth"
            :key="item"
            :label="item"
            :value="item"
          ></el-option>
      ...
      <div class="search-box">
        <input
          ...
          placeholder="请输入关键词"
          class="input-box"
          @keyup.enter="filterAlg((currentPage = 1))"
          :ref="(el) => refElements.push(el)"
        />
...
const getParams = () => {
  return {
    ...
    pageSize: pageSize,
    filing_type:
      filingTypeSelect.value === "全部"
        ? ""
        : filingTypeSelect.value === "大模型"
        ? "model"
        : "algorithm",
    category:
      categoriesSelect.value.slice(0, 2) === "全部"
        ? ""
        : categoriesSelect.value,
    month:
      filingMonthSelect.value.slice(0, 2) === "全部"
        ? ""
        : filingMonthSelect.value,
    region:
      regionsSelect.value.slice(0, 2) === "全部" ? "" : regionsSelect.value,
...
.alg-header {
  ...
  .suanli-btn {
    ...
    width: 280px;
    height: 60px;
    background: url("@/assets/images/suanli-supermarket.png") center center /
      100% 100% no-repeat;
